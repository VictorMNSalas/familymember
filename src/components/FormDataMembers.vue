<template>
    <v-form @submit.prevent>
        <v-container class="spacing-playground mt-1">
            <v-alert v-if="CreatedRecordAlert" class="alert" text="" type="info" variant="tonal">New record
            ...
            <v-row id="row" v-for="(component, index) in componentes" :key="index">
                <v-col cols="12" md="2">
                    <v-select label="Service Type" v-model=rowData[index].service
                        @change="subDepartmentOptions($event, index)" :items="['Immigration', 'Criminal']"></v-select>
                </v-col>
                <v-col cols="12" md="2">
                    <v-select label="Sub Department" v-model=rowData[index].department :items="rowData[index].Departments"
                        @change="subDepartment($event, index)"></v-select>
                </v-col>
                <v-col cols="12" md="2">
                    <v-select label="Name" v-model=rowData[index].name :items="Firstname"
                        @change="setData($event, index)"></v-select>

                </v-col>
                <v-col cols="12" md="2">
                    <v-text-field v-model=rowData[index].phone label="Phone" :counter="12" required></v-text-field>
                </v-col>
                <v-col cols=" 12" md="2">
                    <v-text-field v-model=rowData[index].deposit label="Inicial Deposit" :rules="numeroRules"
                        required></v-text-field>
                </v-col>

                <v-col cols="12" md="1">
                    <v-text-field v-model=rowData[index].fee label="Fee" required :rules="numeroRules"></v-text-field>
                </v-col>
                <v-col cols="1" md="1" class="d-flex justify-center">
                    <v-icon @click="removeItem(index)" size="large" color="red">mdi-delete</v-icon>
                </v-col>
            </v-row>

            <div id="btns">
                <v-btn size="x-large" class="spacing-playground ma-3" @click="duplicarComponente">+</v-btn>
                <v-btn size="x-large" class="spacing-playground ma-3" @click="dataRowGet" :disabled="botondisabled">
        ...
        <v-chip class="ma-10" closable color="blue" text-color="white" prepend-icon="mdi-checkbox-marked-circle">
    ...
    methods: {
        ...
        subDepartmentOptions(data, index) {
            if (data == "Immigration") {
                ...
                this.rowData[index].Departments.push("EOIR - Undetained", "USCIS - U-Cert", "USCIS - Petitions", "SIJS", "FOIA")
            } else if (data == "Criminal") {
                this.rowData[index].Departments = []
                this.rowData[index].Departments.push("Post Conviction - Pre Filing", "Post Conviction - MTV", "Post Conviction - Petitions", "Post Conviction - Investigations", "Post Conviction - Assessments", "Misdemeanors", "Felonies")
            }
        },
        ...
        dataRowGet() {
            this.rowData.forEach((element, index) => {
                this.das.push(index, element.name)

                const fee1 = Number(element.fee);
                const depisit1 = Number(element.deposit);
                if ((element.phone && element.fee && element.deposit && element.name && element.service && element.department) != '') {
